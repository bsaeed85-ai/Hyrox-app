  ...
  return (
    <div
      style={{
        ...
        padding: "10px 12px",
        borderRadius: 10,
        boxShadow: "0 6px 18px rgba(2,132,199,.35)",
        fontSize: 14,
        zIndex: 50,
      }}
    >
      ...
      {items.map((it) => (
        <button
          ...
          style={{
            ...
            borderBottom:
              value === it.value ? "1px solid transparent" : "1px solid #1f2937",
            marginBottom: -1,
            color: "#e5e7eb",
          }}
        >
...
function pacePerKmFrom5k(v) {
  const t = parseTimeToSeconds(v);
  return isFinite(t) ? t / 5 : 300;
}
function ergPace500From1k(v) {
  const t = parseTimeToSeconds(v);
  return isFinite(t) ? t / 2 : 135;
}
function save(k, v) {
  try {
...
function estimateMinutesFromBlocks(blocks) {
  ...
  for (const line of blocks) {
    ...
    if (repsMatch) {
      total += Number(repsMatch[1] || 0) * Number(repsMatch[2] || 0);
      continue;
    }
...
function downloadCSV(filename, rows) {
  const headers = Object.keys(
    rows[0] || { day: "", title: "", blocks: "", focus: "", phase: "" }
  );
  const csv = [
    headers.join(","),
    ...rows.map((r) =>
...
function estimateRaceFromBaseline(b) {
  const run5kSec = parseTimeToSeconds(b.run5k) || sec(25),
    pacePerKm = run5kSec / 5;
  const runFatigue = 1.08,
    run8k = 8 * pacePerKm * runFatigue;
  ...
  const deltas = {
    ski: splits.ski - ref.ski,
    sledPush: splits.sledPush - ref.sledPush,
    sledPull: splits.sledPull - ref.sledPull,
    burpees: splits.burpees - ref.burpees,
    row: splits.row - ref.row,
    farmer: splits.farmer - ref.farmer,
    lunges: splits.lunges - ref.lunges,
    wallballs: splits.wallballs - ref.wallballs,
  };
  const sortedLimiters = Object.entries(deltas)
    .sort((a, b) => b[1] - a[1])
    .map(([k, v]) => ({ station: k, penalty: v }));
...
function stationBlocks(name, variant = "std") {
  const b = {
    ski: [
      "Technique: catch-drive timing 10′",
      "6×250m @ 2k pace; rest 60–75s",
      "1,000m steady @ race RPE 7",
    ],
    row: [
      "Technique: sequencing 8′",
      "5×300m @ 2k pace; rest 60–75s",
      "1,000m steady @ race RPE 7",
    ],
    sledPush: [
      "Warm-up: empty sled 2×20m",
      "4×20m @ moderate; rest 90s",
      "3×20m @ heavy (race+); rest 2′",
    ],
    sledPull: [
      "Warm-up: 2×20m light drag",
      "5×20m @ moderate; rest 90s",
      "3×20m @ heavy; rest 2′",
    ],
    burpees: [
      "Drill: broad jump mechanics 6×3 reps; walk back",
      "6×12 burpee broad jumps; rest 90s",
      "80m continuous @ race effort",
    ],
    farmer: [
      "Carry ladder: 3×(50m light + 30m moderate + 20m heavy); rest 2′",
      "Core brace carries 3×40m suitcase each side",
    ],
    lunges: [
      "Goblet lunge 3×12/leg; rest 90s",
      "100m sandbag walking lunge broken as needed",
    ],
    wallballs: [
      "Warm-up: 2×15 air squats + 10 light wall balls",
      "EMOM 12′: 12 reps",
      "2×50 reps for time; rest 3′",
    ],
  };
    ...
    arr = arr
      .map((l) =>
        l
          ...
          .replace(
            /EMOM *(\d+)[′']/i,
            (m, n) => `EMOM ${Math.max(8, Math.round(Number(n) * 0.8))}′`
          )
      );
  }
  return arr;
...
function annotateBlocks(blocks, p) {
  ...
  const f500 = (s) =>
    `${String(Math.floor(s / 60)).padStart(1, "0")}:${String(Math.round(s % 60)).padStart(2, "0")}/500m`;
  return (blocks || []).map((line) => {
    let out = line;
    out = out.replace(/@ *5k pace(?![^(])/gi, `@ 5k pace (${fkm(p.run5k)})`);
    if (p.goalRun)
      out = out.replace(/@ *(goal|target) pace/gi, `@ target pace (${fkm(p.goalRun)})`);
    out = out.replace(/2k pace(?![^(])/gi, `2k pace (${f500(p.row2k500)})`);
    out = out.replace(
      /SkiErg *1000m|Ski *1000m/gi,
      (m) => `${m} (≈ ${String(secondsToMMSS(Math.round(p.ski500 * 2)))} total)`
    );
    out = out.replace(
      /Row *1000m/gi,
      (m) => `${m} (≈ ${String(secondsToMMSS(Math.round(p.row500 * 2)))} total)`
    );
    return out;
  });
}
function getWeeksToRace(dateStr) {
  ...
  const d = new Date(dateStr + "T00:00:00"),
    now = new Date();
  return Math.ceil((d - now) / (7 * 24 * 3600 * 1000));
}
function getPhase(w) {
  if (w == null) return "Health";
  if (w <= 1) return "Taper";
  if (w <= 4) return "Specific";
  if (w <= 9) return "Build";
  return "Base";
}
...
function makeWeekPlan(athlete, analysis) {
  ...
  const phase =
    athlete.goalType === "Health" || !athlete.raceDate ? "Health" : getPhase(wtr);
  const deload = phase !== "Taper" && isDeloadWeek();
  const top3 = analysis.sortedLimiters.slice(0, 3).map((x) => x.station);
  const focus = (n) => ({
    type: `Station Skill${deload ? " (Deload)" : ""}`,
    focus: n,
    blocks: stationBlocks(n, deload ? "deload" : "std"),
  });
  ...
  const run = {
    easyStrides: {
      type: "Run Easy + Strides",
      blocks: ["35–45′ Z2 run", "6×20s strides, walk back"],
    },
    threshold: {
      type: "Run Threshold",
      blocks: ["Warm-up 10′", "3×10′ @ 5k pace; 2′ easy", "Cooldown 10′"],
    },
    vo2: {
      type: "Run VO2",
      blocks: ["Warm-up 10′", "6×3′ hard; 2′ easy", "Cooldown 10′"],
    },
    racePace: {
      type: "Run Race-Pace",
      blocks: ["Warm-up 10′", "6×1 km @ target pace; 90″ easy", "Cooldown 10′"],
    },
  ...
  const strength = {
    strengthA: {
      type: "Strength A",
      blocks: [
        "Back squat 4×5 @ RPE 7; rest 2–3′",
        "RDL 3×8 @ RPE 7; rest 2′",
        "DB bench 4×6 @ RPE 7",
        "Core: 3×20s hollow + side plank",
      ],
    },
    strengthHybrid: {
      type: "Strength Hybrid",
      blocks: [
        "Front squat 5×3 @ RPE 8; rest 2–3′",
        "Trap bar DL 4×5 @ RPE 7; rest 2′",
        "Push press 5×3 @ RPE 8; rest 2′",
        "Finisher: 10′ easy row",
      ],
    },
    strengthPower: {
      type: "Strength Power",
      blocks: ["Jump squat 5×3 (light, fast)", "Clean pull 4×3 (moderate)", "Push press 4×3"],
    },
  };
  const bricks = {
    ...
    B: {
      type: "Brick B",
      blocks: [
        "2× [1 km @ target pace + 40 m burpee broad + 50 wall balls as 30/20]; 4′",
      ],
    },
    C: { type: "Race Sim Lite", blocks: ["4× [1 km + 2 stations from limiter list]; 3′"] },
    Primer: {
      type: "Race Primer",
      blocks: ["2× [800 m @ target pace + sled push 15 m + sled pull 15 m]; full rest"],
    },
  };

  const days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  function out(plan) {
    return plan.map((s, i) => ({ day: days[i], session: s, phase }));
  }

  if (phase === "Health")
    return out([
      enrich(run.easyStrides),
      focus(top3[0] || "sledPush"),
      strength.strengthA,
      enrich(erg.rowPyramid),
      focus(top3[1] || "burpees"),
      run.longEasy,
      { type: "Recovery", blocks: ["Walk 20–30′", "Mobility 20′"] },
    ]);
  if (phase === "Base")
    return out([
      enrich(run.threshold),
      focus(top3[0] || "sledPush"),
      strength.strengthA,
      enrich(run.easyStrides),
      focus(top3[1] || "burpees"),
      bricks.A,
      { type: "Recovery", blocks: ["Mobility 30–40′", "Easy walk 30′"] },
    ]);
  if (phase === "Build")
    return out([
      enrich(run.vo2),
      focus(top3[0] || "sledPush"),
      strength.strengthHybrid,
      focus(top3[1] || "wallballs"),
      enrich(erg.skiFastFinish),
      bricks.B,
      run.longEasy,
    ]);
  if (phase === "Specific")
    return out([
      ...
      focus(top3[1] || "sledPull"),
      enrich(erg.rowFastFinish),
      bricks.C,
      { type: "Recovery", blocks: ["Mobility 30–40′", "Easy walk 30′"] },
    ]);
  if (phase === "Taper")
    return out([
      enrich(run.easyStrides),
      bricks.Primer,
      { type: "Recovery", blocks: ["30–35′ recovery + mobility"] },
      { type: "Tune-up", blocks: ["20′ easy + 4×10″ pickups"] },
      { type: "Activation", blocks: ["10′ easy + 10 wall balls + 10 burpees"] },
      { type: "Off / Travel", blocks: ["Gentle walk, hydration focus"] },
      { type: "Recovery", blocks: ["Mobility 20′"] },
    ]);
  return [];
}
...
function AuthBar() {
  if (!supa)
    return (
      <div className="card">
        <div className="muted">
          Supabase not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Vercel → Environment
          Variables.
        </div>
      </div>
    );
...
/* ---------- Guided Baseline ---------- */
function Stopwatch({ running, onToggle, onReset, seconds }) {
  const mmss = secondsToMMSS(seconds);
  return (
    <div className="row">
      <div style={{ fontFamily: "monospace", fontSize: 24 }}>{mmss}</div>
...
function GuidedBaseline({ base, setBase }) {
  const tests = [
    ["run5k", "Run 5k"],
    ["ski1k", "SkiErg 1,000 m"],
    ["row1k", "Row 1,000 m"],
    ["sledPush50m", "Sled Push 50 m"],
    ["sledPull50m", "Sled Pull 50 m"],
    ["burpeeBroad80m", "Burpee Broad 80 m"],
    ["farmer200m", "Farmer’s Carry 200 m"],
    ["lunges100m", "Walking Lunges 100 m"],
  ];
  const [idx, setIdx] = React.useState(0);
  const [running, setRunning] = React.useState(false);
  const [secElapsed, setSecElapsed] = React.useState(0);
  React.useEffect(() => {
    if (!running) return;
    const id = setInterval(() => setSecElapsed((s) => s + 1), 1000);
    return () => clearInterval(id);
  }, [running]);
  const atEnd = idx >= tests.length;
  const curr = tests[idx] || null;
  function saveTime() {
    const mmss = secondsToMMSS(secElapsed);
    const k = tests[idx][0];
    setBase({ ...base, [k]: mmss });
  }
  return (
    <Card title="Guided Baseline">
      <div className="muted" style={{ marginBottom: 8 }}>
        Use the stopwatch to time each test. Click Save after each, then Next.
      </div>
      {!atEnd ? (
        <>
          ...
          <div className="muted" style={{ marginTop: 8 }}>
            Saved: {tests.map((t) => (base[t[0]] ? t[1] : null)).filter(Boolean).join(", ") || "None"}
          </div>
        </>
      ) : (
        <>
...
/* ---------- This Week (cloud) ---------- */
function ThisWeek({ user, onToast }) {
  ...
  async function toggleComplete(row) {
    ...
    setGroups((g) =>
      g.map((grp) => ({
        ...grp,
        items: grp.items.map((it) => (it.id === row.id ? { ...it, completed: newVal } : it)),
      }))
    );
    ...
    if (error) {
      ...
      setGroups((g) =>
        g.map((grp) => ({
          ...grp,
          items: grp.items.map((it) => (it.id === row.id ? { ...it, completed: row.completed } : it)),
        }))
      );
    } else if (onToast) {
      onToast(newVal ? "Marked completed ✅" : "Marked not completed");
    }
  }
          ...
          return (
            <div key={grp.day_index} className="card" style={{ padding: 12 }}>
              <div
                ...
                onClick={() => toggleAccordion(grp.day_index)}
              >
                ...
                <div style={{ marginTop: 8, display: "grid", gap: 8 }}>
                  {grp.items.map((w) => (
                    <div
                      key={w.id}
                      className="card"
                      style={{ padding: 12, opacity: w.completed ? 0.6 : 1 }}
                    >
                      <div className="row" style={{ justifyContent: "space-between" }}>
                        <div>
...
/* ---------- Train Today ---------- */
function TrainTodayCard({ user, onToast }) {
  ...
  async function save() {
    ...
    setMsg("Saved.");
    if (onToast) onToast("Session saved ✅");
  }
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                placeholder="e.g., Run Threshold 3x10"
                value={title}
...
function HistoryPage({ user }) {
  ...
  // PR form state
  const [prExercise, setPrExercise] = React.useState("");
  const [prValue, setPrValue] = React.useState("");
  const [prValueNum, setPrValueNum] = React.useState("");
  const [prUnit, setPrUnit] = React.useState("");
  const [prIsTime, setPrIsTime] = React.useState(false);
  ...
  const weekStats = React.useMemo(() => {
    const arr = weekKeys.map((wk) => {
      ...
      const avgReadiness = total
        ? Math.round((rows.reduce((a, r) => a + (r.readiness || 0), 0) / total) * 10) / 10
        : 0;
  ...
  // Derive PR “bests”
  const bestByExercise = React.useMemo(() => {
    const map = new Map();
    for (const r of prs) {
      const key = r.exercise || "Unknown";
      const existing = map.get(key);
      if (!existing) map.set(key, r);
      else {
        if (r.is_time) {
          // lower value_num is better when time is stored, else try parsing
          const a = Number(existing.value_num ?? parseTimeToSeconds(existing.value));
          const b = Number(r.value_num ?? parseTimeToSeconds(r.value));
          if (isFinite(b) && (!isFinite(a) || b < a)) map.set(key, r);
        } else {
          const a = Number(existing.value_num ?? existing.value);
          const b = Number(r.value_num ?? r.value);
          if (isFinite(b) && (!isFinite(a) || b > a)) map.set(key, r);
        }
      }
    }
  ...
  async function savePR() {
    if (!supaLocal || !user?.id) return alert("Sign in first.");
    if (!prExercise.trim()) return alert("Add an exercise name.");
    setSavingPr(true);
    const payload = {
      user_id: user.id,
      exercise: prExercise.trim(),
      value: prValue || null,
      value_num: prValueNum === "" ? null : Number(prValueNum),
      unit: prUnit || null,
      is_time: !!prIsTime,
    };
    const { error } = await supaLocal.from("prs").insert(payload);
    setSavingPr(false);
    ...
    <section className="border border-slate-800 rounded-xl p-4">
      ...
      <div className="card" style={{ padding: 12, marginBottom: 12 }}>
        <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
          ...
          />
          <input
            placeholder="Value Num (for charts)"
            type="number"
            value={prValueNum}
            onChange={(e) => setPrValueNum(e.target.value)}
            style={{ width: 140 }}
          />
          <input
            placeholder="Unit (kg, reps, m)"
            value={prUnit}
          ...
          <label className="row" style={{ gap: 6 }}>
            <input
              type="checkbox"
              checked={prIsTime}
              onChange={(e) => setPrIsTime(e.target.checked)}
            />
            <span className="muted">Is time</span>
          </label>
          <button className="btn" onClick={savePR} disabled={savingPr}>
      ...
      {loading ? (
        ...
        <>
              ...
              <div key={wk}>
                ...
                <div className="w-full h-2 bg-slate-800 rounded">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.round((mins / weekStats.maxMins) * 100))}%`,
                    }}
                  />
                </div>
              </div>
          ...
          {/* PRs with best badges and simple “trend” */}
          <div className="mt-6">
            ...
            {prs.length === 0 ? (
              ...
              <ul className="text-sm grid md:grid-cols-2 gap-2">
                {prs.map((pr) => {
                  const best = bestByExercise.get(pr.exercise || "Unknown");
                  const isBest = best && best.id === pr.id;
                  return (
                    <li
                      key={pr.id}
                      className="border border-slate-800 rounded px-2 py-1 flex items-center justify-between"
                    >
                      <div>
                      ...
                      <div className="font-mono">
                        {pr.is_time
                          ? secondsToMMSS(pr.value_num ?? parseTimeToSeconds(pr.value))
                          : pr.value}
                        {pr.unit ? ` ${pr.unit}` : ""}
                      </div>
                    </li>
...
export default function App() {
  // Public page route (no share page in this build; keeping hook for future)
  ...
  const [athlete, setAthlete] = React.useState(
    load("hyrox.athlete", { name: "", division: "Open", goalType: "Race", raceDate: "" })
  );
  ...
  const [profile, setProfile] = React.useState(
    load("hyrox.profile", { experience: "beginner", goal: "balanced" })
  );
  ...
  const plan = React.useMemo(
    () => makeWeekPlan({ goalType: athlete.goalType, raceDate: athlete.raceDate, ...base }, analysis),
    [athlete, base, analysis]
  );
  ...
  React.useEffect(() => {
    ...
    (async () => {
      const { data } = await supa.auth.getSession();
      setUser(data?.session?.user || null);
      const res = supa.auth.onAuthStateChange((_e, s) => setUser(s?.user || null));
      sub = res.data?.subscription;
    })();
    return () => sub?.unsubscribe?.();
  }, []);
  ...
  async function generateWeeksToCloud(nWeeks = 4, startWeek = 1) {
    ...
    const { error } = await supa.from("workouts").upsert(rows, {
        ...
        {tab === "dashboard" && (
          <>
            <Card title="Athlete">
              <div className="row">
                ...
                <select
                  ...
                  onChange={(e) => setAthlete({ ...athlete, division: e.target.value })}
                >
                  <option>Open</option>
                  <option>Pro</option>
                ...
                <select
                  value={athlete.goalType}
                  onChange={(e) => setAthlete({ ...athlete, goalType: e.target.value })}
                >
                ...
                <input
                  ...
                  onChange={(e) => setAthlete({ ...athlete, raceDate: e.target.value })}
                />
              </div>
            </Card>
            ...
            <Card
              ...
              right={
                <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
                  <button className="btn" onClick={() => generateWeeksToCloud(1, 1)}>
                    Save Week 1 → Cloud
                  </button>
                  <button className="btn" onClick={() => generateWeeksToCloud(4, 1)}>
                    Save 4 Weeks → Cloud
                  </button>
                  <button
                    className="btn"
            ...
            >
              <div className="row" style={{ flexWrap: "wrap" }}>
                {[
                  ...
                  ["burpeeBroad80m", "Burpee broad 80m"],
                  ["farmer200m", "Farmers 200m"],
                  ["lunges100m", "Lunges 100m"],
                ].map(([k, label]) => (
                  <input
                    key={k}
                    placeholder={label}
                ...
                <input
                  type="number"
                  placeholder="Wall-balls rate (reps/min)"
                  value={base.wallballs100}
                  onChange={(e) => setBase({ ...base, wallballs100: Number(e.target.value) })}
                />
                <div className="row" style={{ alignItems: "center" }}>
                  ...
                  <input
                    ...
                    min={1}
                    max={5}
                    value={base.readiness}
                    onChange={(e) => setBase({ ...base, readiness: Number(e.target.value) })}
                  />
                </div>
              </div>
            </Card>
            ...
            <Card title="Race Twin (prediction)">
              <div className="row" style={{ gap: 16, flexWrap: "wrap" }}>
                <div className="pill">
                  Predicted: <b>{secondsToMMSS(analysis.total)}</b>
                </div>
                <div className="muted">
                  Limiters: {analysis.sortedLimiters.slice(0, 4).map((x) => x.station).join(", ")}
                </div>
              </div>
              <hr />
            ...
            <TrainTodayCard user={user} onToast={pushToast} />
            <ThisWeek user={user} onToast={pushToast} />
